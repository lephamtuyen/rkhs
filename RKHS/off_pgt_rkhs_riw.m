function off_pgt_rkhs_riw
clear all; %#ok<CLFUN>
close all;
clc;
global lambda gamma domain A_dim Episodes Steps numRuns MaxIter evel_episodes
...
for i=1:numRuns
    ...
    while (e <= MaxIter)
        ...
        % Get Q approx
        Q_approx = getLSTDQApprox(policy,data);

        % ///////////////////////////// RKHS //////////////////////////////////
        policy_new = RKHS(policy,data,Q_approx,regularizers);
...
%         lambda = 0.1;
        ...
        evaluate = getTrajectory(policy, evel_episodes, Steps);
        rew(i,e) = evaluate.averageReward;
        
        if (evaluate.averageReward > data(end).averageReward)
            regularizers = max(regularizers/5,1e0);
        else
            policy = data(end).policy;
            regularizers = min(regularizers*5,1e2);
            data(end) = getTrajectory(policy, Episodes, Steps);
        end
...
filename = strcat('data/revision/',domain,'_off_pgt_rkhs_riw');
save(filename,'rew');
end
...
global lambda_range A_dim evel_episodes Steps
...
%     data_new  = obtainData(temp_policy,[],evel_episodes);
    data_new = getTrajectory(temp_policy, evel_episodes, Steps);
...
function [new_policy] = RKHS(policy, data, Q_approx, regularizers)
...
for poli=1:size(data,1)
    for epi = 1 : data(poli).numOfEpisodes
        ...
        iws(count_epi) = iw;count_epi = count_epi + 1;
        J_hat = J_hat + iw*Q_approx(idx:idx+Steps-1);
        
        new_policy.params(:,i:i+Steps-1)=[dLogPi;s];
        i = i + Steps;idx=idx+Steps;
    end
end

J_hat = J_hat / (sum(iws)+1e-10);
i=policy_size+1;
count_epi = 1;idx = 1;
for poli=1:size(data,1)
    for epi = 1 : data(poli).numOfEpisodes
...
%         R = sum(bsxfun(@times,data(poli).episodes(epi).r,gammas));
        ...
        new_policy.params(1:A_dim,i:i+Steps-1) = ...
            bsxfun(@times, iws(count_epi)/(sum(iws)+1e-10), bsxfun(@times,Q_approx(idx:idx+Steps-1)-J_hat+regularizers,new_policy.params(1:A_dim,i:i+Steps-1)));
...
% function [new_policy] = RKHS(policy, data,Q_approx)
% global A_dim S_dim gamma
% 
% new_policy = policy;
% new_policy.id = getPolicyID;
...
%         end
% 
%         grad = bsxfun(@times,iw*(Q_approx(idx:idx+Steps-1))/numOfEpisodes,dLogPi);
% 
...
for poli=1:size(data,1)
    for epi = 1 : data(poli).numOfEpisodes
        % first action state
        r = data(poli).episodes(epi).r(1);
        
        k = Kernel(i,:);
        i = i + 1;
        for step=1:data(poli).length(epi)
            if (step == data(poli).length(epi)) % last step
                A = A + (k'*k)  + 0.000000000001;
                b = b + (k'*r);
            else
                rn = data(poli).episodes(epi).r(step+1);
                kn = Kernel(i,:);
                i = i + 1;
...
function Q_approx = getLSTDQApprox(policy,data)
% Perform sparsification
D = ALDSparsification(policy, data);
