function off_lr_rkhs_riw
clear all; %#ok<CLFUN>
close all;
clc;

global lambda gamma domain A_dim evel_episodes numRuns MaxIter Steps Episodes
...
regu = zeros(numRuns,MaxIter);
% regu = [1000;100;50;30;10;1;0;0.1;0.01;0.001];
% for xxx=1:10
for i=1:numRuns
    fprintf('RUN: %d\n', i);
...
%         data = find_optimal_sampling_policy(data);
        
        % ///////////////////////////// RKHS //////////////////////////////////
        policy_new = RKHS(policy, data, regularizers);
...
%         lambda = 0.005;
        ...
        regu(i,e) = regularizers;

        if (evaluate.averageReward > data(end).averageReward)
            regularizers = max(regularizers/5,1);
        else
...
% filename = strcat('data/',domain,'_off_lr_rkhs_riw__',int2str(xxx));
filename = strcat('data/revision/',domain,'_off_lr_rkhs_riw_05');
save(filename,'rew');
% filename = strcat('data/',domain,'_regu');
% save(filename,'regu');
end
% end
...
function [new_policy] = RKHS(policy, data, regularizers)
...
for poli=1:size(data,1)
    for epi = 1 : data(poli).numOfEpisodes
        ...
        new_policy.params(1:A_dim,i:i+Steps-1) = ...
            bsxfun(@times, iws(count_epi)/(sum(iws)+1e-10), bsxfun(@times,R-J_hat+regularizers,new_policy.params(1:A_dim,i:i+Steps-1)));
...
%                 bsxfun(@times, iws(count_epi)/(sum(iws)+1e-10), bsxfun(@times,R-J_hat+regularizers,new_policy.params(1:A_dim,i:i+Steps-1)));
%         else
%             new_policy.params(1:A_dim,i:i+Steps-1) = (1/numOfEpisodes)*new_policy.params(1:A_dim,i:i+Steps-1);
%         end
%         i = i + Steps;
%         count_epi = count_epi + 1;
%     end
