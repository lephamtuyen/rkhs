function off_pgt_rkhs_uiw
clear all; %#ok<CLFUN>
close all;
clc;
global lambda gamma domain A_dim Episodes Steps numRuns MaxIter
...
for i=1:numRuns
    ...
    while (e <= MaxIter)
        ...
        % ///////////////////////////// RKHS //////////////////////////////////
        policy_new = RKHS(policy,data,Q_approx);
...
filename = strcat('data/',domain,'_off_pgt_rkhs_uiw');
save(filename,'rew');
end
...
%     dLogPi = pinv(diag(policy.sigma.^2))*(a - hs);
% 
%     grad = bsxfun(@times, data.iw(idx:idx+Steps-1), bsxfun(@times,data.Q_approx(idx:idx+Steps-1)/data.numOfEpisodes,dLogPi));
% 
%     new_policy.params(:,i:i+Steps-1)=[grad;s];
%     i = i + Steps;idx=idx+Steps;
% end
